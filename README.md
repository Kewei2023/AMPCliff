...
✅the TaiChiNet2 is in `taichinet` folder

you can set parameters in `configs/downstream.yaml`
```python 
model:
  # ... ignore some settings
  regression:
    version: esm2_t6 # gpt2-base # progen2-medium # progen2-base # esm2_t12 # progen2-small # gpt2-large # gpt2-base # esm2_t33 # gpt2-large # bert-base # protgpt2 # CellFree-cnn # CellFree-rnn # AMPSpace # v1 # AMPSpace, CellFree-cnn, SeqUNet
    bagua: true # multi-combinations of real and image
    inter_layer:
      save_type: image
      pool: max
    intra_layer:
...
### Supercomputer: evaluation scripts (env overrides)

Grid scripts under `evaluation_scripts/` (e.g. `run_baseline_pooling_grid_2x2_seeds.sh`, `run_local_stft_grid_2x2_seeds.sh`) default to a fixed `REPO_ROOT` that you can override: `export REPO_ROOT=/path/to/your/AMPCliff`. They set `OUTPUT_ROOT` to `${REPO_ROOT}/outputs/ablation` unless you `export OUTPUT_ROOT=...`.

Training helpers such as `run_baseline_pooling_train.sh` default ESM2 `CONFIG_DIR` to `/data/public/models/facebook/...` when unset (same as `factory/initializer.py`). Override with `export CONFIG_DIR=/your/esm2/checkpoint/dir/` if your cluster layout differs.
