...
def smith_waterman(seq1, seq2, scoring_matrix=substitution_matrices.load("BLOSUM62"), gap_open=-10, gap_extend=-0.5, lambda_value=0.251, K=0.031):
...
def process_sequence_pair(seq_pair,diff,models):
    ...
    if dmic > np.log10(diff):
      ...
      for model in models:
        pred_mic1,pred_mic2 = seq1[f'{model}-pred'],seq2[f'{model}-pred']
        pred_dmic = pred_mic1-pred_mic2
        acindex = pred_dmic/dmic-1 # closer to 0 is better
        
        new_row[f'{model}-pred_mic1'],new_row[f'{model}-pred_mic2'],new_row[f'{model}-pred_dmic']=pred_mic1,pred_mic2,pred_dmic
        new_row[f'{model}-acindex'] = acindex
...
def seq2pair(testset,diff):
    ...
    models = [_.split('-')[0] for _ in testset.columns if 'pred' in _]
    new_rows = []
    
    for pair in sequence_pairs:
      new_row = process_sequence_pair(pair,diff,models)
